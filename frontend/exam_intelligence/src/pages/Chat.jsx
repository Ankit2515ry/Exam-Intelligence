  ...
  const handleSend = async (e) => {
    ...
    try {
      const response = await chatService.sendMessage(
        currentInput,
        selectedDocId ? String(selectedDocId) : null
      );
      ...
      setMessages((prev) => [
        ...
        {
          role: "bot",
          text: "Authentication or server context routing pipeline error.",
        },
      ]);
    } finally {
      setLoading(false);
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col p-4 justify-between">
        ...
        <div className="p-2 bg-slate-800 rounded-xl border border-slate-700 my-4">
          ...
          <select
            ...
            className="w-full bg-slate-900 text-white border border-slate-700 rounded-md p-1.5 text-xs focus:outline-none"
          >
            <option value="">Global System Scope</option>
      ...
      <main className="flex-1 flex flex-col bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50">
          {messages.map((msg, index) => (
            ...
            >
              <div
                className={`p-2 rounded-full h-8 w-8 flex items-center justify-center border shrink-0 ${
                  msg.role === "user"
              ...
              <div
                className={`p-3 rounded-2xl text-sm leading-relaxed shadow-xs border ${
                  msg.role === "user"
                ...
                {msg.role === "bot" &&
                  ...
                  msg.sources.length > 0 && (
                    <div className="mt-4 border-t pt-3">
                      <h4 className="font-semibold text-xs uppercase text-slate-500 mb-2">
                        Sources
                      </h4>
                      ...
                      {msg.sources.map((source) => (
                        <div
                          key={source.chunk_id}
                          className="mb-3 p-2 bg-slate-50 rounded border"
                        >
          ...
          {loading && (
            <div className="flex gap-3 max-w-2xl">
              ...
              <div className="p-3 rounded-2xl text-sm bg-white border border-slate-200 text-slate-400 italic rounded-tl-none">
                Agent is processing vectors...
              </div>
            </div>
          )}
        </div>
        ...
        <footer className="p-4 border-t border-slate-200 bg-white">
          <form onSubmit={handleSend} className="flex gap-2">
            <input
              ...
              placeholder="Submit a question related to your uploaded documents..."
              className="flex-1 border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
            />

            <button
              type="submit"
              disabled={loading}
              className="bg-indigo-600 text-white p-2.5 rounded-lg hover:bg-indigo-700 transition disabled:opacity-50"
            >
              <Send size={18} />
            </button>
          </form>
